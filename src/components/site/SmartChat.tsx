...
export function SmartChat() {
  ...
  const submit = (event: FormEvent) => {
    ...
    if (!text) return;
    const answer = answerSiteQuestion(text, siteConfig);
    setMessages((current) => [
      ...current,
      { from: "user", text },
      { from: "bot", text: answer.text, source: answer.source },
    ]);
    setQuestion("");
  };
      ...
      {open ? (
        <>
          <button
            ...
            className="fixed inset-0 z-[55] cursor-default bg-black/5 backdrop-blur-[1px]"
          />
          <section
            role="dialog"
            aria-modal="true"
                ...
                >
                  ...
                  {message.source ? (
                    <a
                      ...
                      className="mt-2 inline-flex font-semibold text-primary underline decoration-primary/35 underline-offset-4 hover:decoration-primary"
                    >
